...
const getTeddies = async function () {
  try {
    ...
    if (reponse.status === 200) {
      ...
      // Récupération de la div avec id produit-personnaliser pour mettre la personnalisation du produit
      ...
      imgProduit.setAttribute(
        "alt",
        "Ours en peluche qui s'appelle " + teddies.name
      );
      ...
      ajoutPanier.addEventListener("click", function (event) {
        ...
        //Enregistrement des données dans le local storage (panier)
        let enregistrementTeddy = JSON.parse(
          localStorage.getItem("nouvelArticle")
        ); // la valeur "enregistrementTeddy" est envoyée à la clé "nouvelArticle"
        if (enregistrementTeddy) {
          ...
          if (
            window.confirm(
              teddies.name +
                " a été ajouté à votre panier. pour aller sur votre panier cliquez sur OK sinon cliquez sur Annuler pour revenir à la page d'accueil"
            )
          ) {
